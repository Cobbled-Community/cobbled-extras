...
import xyz.nucleoid.extras.mixin.PersistentEntitySectionManagerAccessor;
import xyz.nucleoid.extras.mixin.ServerLevelAccessor;
    ...
    private void tick(MinecraftServer server) {
        ...
        if (time - this.lastSendTime > SEND_INTERVAL_MS) {
            ...
            for (var world : server.getAllLevels()) {
                var entityManager = ((ServerLevelAccessor) world).nucleoid$getEntityManager();
                var entityIds = ((PersistentEntitySectionManagerAccessor) entityManager).nucleoid$getKnownUuids();
